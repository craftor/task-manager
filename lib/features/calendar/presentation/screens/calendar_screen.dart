...
class CalendarScreen extends ConsumerWidget {
          ...
          moodsAsync.when(
            data: (moods) {
              ...
              return Container(
                ...
                child: IconButton(
                  icon: Text(
                    todayMoods.isEmpty ? '😶' : todayMoods.join(''),
                    style: const TextStyle(fontSize: 18),
                  ),
                  onPressed: () => _showMoodPicker(context, ref, calendarState.selectedDay, todayMoods),
                  tooltip: todayMoods.isNotEmpty ? 'Change mood' : 'Add mood',
                ),
              );
            },
  ...
  Widget _buildCalendarBody(BuildContext context, WidgetRef ref, List<Task> tasks, CalendarState calendarState) {
    return Column(children: [
      _buildCalendar(context, ref, tasks, calendarState),
      const Divider(height: 1, color: AppColors.border),
      Expanded(child: _buildTasksForSelectedDay(context, ref, tasks, calendarState.selectedDay)),
    ]);
  }

  Widget _buildCalendar(BuildContext context, WidgetRef ref, List<Task> tasks, CalendarState calendarState) {
    final moodsAsync = ref.watch(allMoodsProvider);
    ...
    return TableCalendar<Task>(
      ...
      onPageChanged: (focusedDay) { ref.read(calendarProvider.notifier).setFocusedDay(focusedDay); },
      calendarBuilders: CalendarBuilders(
        defaultBuilder: (context, date, focusedDay) {
          final key = _dateKey(date);
          final moodList = moods[key];
          ...
          Color textColor;
          FontWeight weight;
          if (isSelected) { textColor = AppColors.background; weight = FontWeight.w600; }
          else if (isToday) { textColor = AppColors.textPrimary; weight = FontWeight.w700; }
          else if (isOutside) { textColor = AppColors.textSecondary; weight = FontWeight.w400; }
          else if (date.weekday == 6 || date.weekday == 7) { textColor = AppColors.textSecondary; weight = FontWeight.w500; }
          else { textColor = AppColors.textPrimary; weight = FontWeight.w400; }

          return Container(
            alignment: Alignment.center,
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text('${date.day}', style: TextStyle(color: textColor, fontSize: 14, fontWeight: weight)),
              if (moodList != null && moodList.isNotEmpty)
                Text(moodList.join(''), style: const TextStyle(fontSize: 11)),
            ]),
          );
        },
        markerBuilder: (context, date, events) {
          if (events.isEmpty) return null;
          return Positioned(bottom: 1, child: Row(mainAxisSize: MainAxisSize.min,
            children: events.take(3).map((task) {
              final color = _getPriorityColor(task.priority);
              return Container(margin: const EdgeInsets.symmetric(horizontal: 1), width: 5, height: 5,
  ...
  Widget _buildTasksForSelectedDay(BuildContext context, WidgetRef ref, List<Task> tasks, DateTime selectedDay) {
    ...
    if (dayTasks.isEmpty) {
      return const Center(child: Column(children: [
        ...
        SizedBox(height: 16),
        Text('No tasks due on this day', style: TextStyle(color: AppColors.textSecondary)),
      ]));
    }

    return ListView.builder(padding: const EdgeInsets.all(16), itemCount: dayTasks.length,
      itemBuilder: (context, index) => _CalendarTaskCard(task: dayTasks[index]));
  }

  void _showMoodPicker(BuildContext context, WidgetRef ref, DateTime day, List<String> currentMoods) {
...
class _CalendarTaskCard extends StatelessWidget {
  ...
  String _formatDateTime(DateTime dt) => '${dt.day}/${dt.month} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}';
